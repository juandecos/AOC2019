...
namespace AOC2019
{
    class AOC9_1
    {
        ...
        public static long GetAddress(long[] data, long position, long relativeBase, int opMode)
        {
            ...
            {
                return data[position] + relativeBase;
            }

            return position; // Immediate mode
        }

        public static void Run(long[] inputs)
        {
            long inputPosition = 0;
            long relativeBase = 0;
            long[] programData = input.Split(',').Select(x => long.Parse(x)).ToArray();
            long[] data = new long[10000];
            for (int index = 0; index < programData.Length; index++)
                data[index] = programData[index];
            for (long position = 0; position < data.Length;)
            {
                long opcode = data[position];
                position++;
                ...
                int[] opModes = Enumerable.Range(0, 3).Select(x => ((int)opcode / (int)Math.Pow(10, x + 2) % 10)).ToArray();
                opcode = opcode % 100;

                if (opcode == 3)
                {
                    long writeAddress = GetAddress(data, position, relativeBase, opModes[0]);
                    position++;
                    data[writeAddress] = inputs[inputPosition];
                    inputPosition++;
                    continue;
                }

                long op1 = GetAddress(data, position, relativeBase, opModes[0]);
                position++;

                if (opcode == 4)
                {
                    Console.Write(data[op1]);
                    continue;
                }

                if (opcode == 9)
                {
                    relativeBase += data[op1];
                    continue;
                }

                long op2 = GetAddress(data, position, relativeBase, opModes[1]);
                position++;
                ...
                long op3 = GetAddress(data, position, relativeBase, opModes[2]);
                position++;
